...
import {
  getFacultyApplicationDraftState,
...
export default defineEventHandler(async (event) => {
  ...
  const state = await getFacultyApplicationDraftState(tablesDB, config.value, sessionUser.$id)

  return {
    ok: true,
    editable: state.editable,
    application: state.application ? mapFacultyApplicationRow(state.application) : null,
    documents: state.documents.map(mapFacultyApplicationDocumentRow)
  }
})
